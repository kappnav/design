...
| kappnav.app.auto-create.kinds | Specifies a json array of resource kinds to be included in application component list. | [**Deployment**, **DeploymentConfig**, **DaemonSet**, **StatefulSet**, **(Knative) Service**, **(K8S Core) Service**, **Ingress**, **ConfigMap**] if the kappnav.app.auto-create=true label is present, otherwise [**Deployment**, **DeploymentConfig**, **DaemonSet**, **StatefulSet**, **(Knative) Service**] | 
...
| kappnav.app.auto-create.labels-values |	Specifies a json array of match values for auto-create label. | The value of the **app.kubernetes.io/part-of** label, otherwise the name of the associated application resource if the label is not present. If one value is specified, the label selector is matchLabels. If multiple values are specified, the label selector is matchExpression. |
